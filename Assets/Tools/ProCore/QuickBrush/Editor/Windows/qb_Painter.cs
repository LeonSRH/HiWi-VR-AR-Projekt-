	...
	void OnGUI()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (GUILayout.Button("", picButton_TemplateActive, GUILayout.Width(16), GUILayout.Height(16)))
			{
				...
				UpdateCompoundPaintToSelection();
				window.Repaint();
			} DoTipCheck("Toggle Template in slot '" + i + "' IsActive/Inactive." + " If no slot is checked, the currently selected slot is isActive");
...
#pragma warning disable CS0618 // "Handles.ArrowCap(int, Vector3, Quaternion, float)" ist veraltet: "Use ArrowHandleCap instead"
...
#pragma warning restore CS0618 // "Handles.ArrowCap(int, Vector3, Quaternion, float)" ist veraltet: "Use ArrowHandleCap instead"
			}
		}
	...
	static bool areActive = false; //simple bool to show whether there are any templates marked as isActive
	static void UpdateAreActive() //This should be run on enable, whenever isActive state is modified, and whenever tab operations are performed
	{
		int count = window.brushTemplates.Length;
	...
	static void UpdateStroke() // this could be called up if isActive templates has any trues otherwise go with the default path if can do without doubling up functions
	{
		int count = window.brushTemplates.Length;
		int curIndex = window.liveTemplateIndex;
		...
		//Check if any templates are isActive
		if (areActive == true)
			for (int i = 0; i < count; i++)
			{
...
#pragma warning disable CS0253 // Unbeabsichtigter Verweisvergleich. Wandeln Sie die rechte Seite in den Typ "Object" um, um einen Wertvergleich durchzuführen.
					if (PrefabUtility.GetCorrespondingObjectFromSource(objects[ii]) != curPrefab)
#pragma warning restore CS0253 // Unbeabsichtigter Verweisvergleich. Wandeln Sie die rechte Seite in den Typ "Object" um, um einen Wertvergleich durchzuführen.
						addToList = false;
				}
	...
	//We can just update the Compound Layer Mask when the layer menu is changed, when the IsActive State is changed, and when Switching Tabs
	...
	private static void UpdateCompoundLayerMask()
	{
		...
		//get the isActive template's mask
...
//		}
		...
		//if any others are isActive get their masks too
		if (areActive == true)
			for (int i = 0; i < count; i ++)
			{
				//			if(i == window.liveTemplateIndex)
